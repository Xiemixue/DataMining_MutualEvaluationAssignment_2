...
data = pd.read_csv('archive/USvideos.csv', sep=',')

# 转化category_id
with open('archive/US_category_id.json') as f:   # 读取json文件
    json_date = js.load(f)
    f.close()
id2cat = {}
...
for i in data['views']:
    if i == 'low view':
        low_view_cnt += 1
print("like项数量={}, low view项数量={}".format(like_cnt, low_view_cnt))
...
sns.heatmap(data=conf_pd, annot=True, fmt=".2f", cmap="Blues")
plt.show()
...
plt.figure(figsize=(11, 9),dpi=100)
sns.heatmap(data = lift_pd, annot = True, fmt = ".2f", cmap = "Blues")
plt.show()
...
plt.figure(figsize=(11, 9), dpi=100)
sns.heatmap(data=kulc_pd, annot=True, fmt=".2f", cmap="Blues")
plt.show()